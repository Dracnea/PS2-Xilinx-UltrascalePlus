...
### The Graphics Synthesizer — 1 of 4 and one partial, updated 2026-09-17
...
| **texture unit** | *wired into the rasteriser 2026-09-17* — `gs_texaddr.vhd` does the addressing for all nine formats and the four wrap modes (29,140 cases on five seeds, eleven mutations caught, 349 LUTs) `gs_clut.vhd` the palette and its CLD reload rules (78 cases on six seeds, fourteen mutations, 170 LUTs and one block RAM), and `gs_texsample.vhd` the fetch, palette lookup and texture function (498 samples on four seeds, eleven mutations, 1327 LUTs) and `gs_texcache.vhd` the texel cache that pays for it (498 samples plus a raster walk A/B'd against an uncached sampler, twelve mutations, 1570 LUTs and **no block RAM and no UltraRAM**) — **step 4 of gs-texture.md is complete, so a 2D textured primitive can be drawn**. **Nearest and bilinear** — the filter is 1257 samples on four seeds (498 nearest, 759 filtered) and sixteen mutations, for +262 LUTs; 16-bit CLUTs are refused rather than guessed, and MMAG/MMIN selection is wired through the LOD. **The perspective divide is built**: `gs_stq.vhd`, 1986 coordinates identical to `stq_to_uv` on four seeds and seventeen mutations, 786 LUTs and no DSP, 29 clocks. Nothing interpolates S, T and Q across a primitive yet, so PRIM.FST = 0 is still refused. **The one read port is decided**: 64 sets × 2 ways of 256-bit lines with an XOR-folded index, which puts a raster walk exactly on its compulsory-miss floor on all four walks — 87 % and 96 % nearest, **96 % and 99 % bilinear**, so a filtered pixel costs 1.95× an unfiltered one rather than 4×, which is the ratio the real GS has. **The GS draws textured triangles**, byte-identical to the model on four random GIF streams: UV interpolated on two more `gs_chan_dda`, a texel per pixel through the cache, TFX, and the LOD choosing between MMAG and MMIN. PRIM.FST=0 and textured sprites are counted and refused, not drawn wrong. The whole GS is 29,007 LUTs and **still 128 UltraRAM**. **It runs on the C1100**: a textured 64 x 64 frame identical to the model on all 4096 pixels through three read paths, with nearest and bilinear+0.5 pixel-identical at 1:1 (the half-texel shift, measured) and 204 of 1024 differing at 2x (the filter filtering), and 297,276 display reads with none refused by the arbiter. The reference model is **complete**: `gs-texture.md` runs from the PSMT8/PSMT4 address permutations through the H formats and the CLUT to UV sampling, STQ with the perspective divide, mipmap and bilinear. The RTL will have an exhaustively cross-checked oracle to diff against from its first line |
