...
| `c1100_ps2_iop.bit` | `35cdc402db00473042f1fed51fb9d2c7` | xcu55n-fsvh2892-2LV-e | `boards/c1100_ps2_iop.py` — a LitePCIe gen3 x4 endpoint, the IOP subsystem on CSRs, HBM with three masters (the disc source, the IOP, and the host-to-card DMA writer) | **Rebuilt 2026-09-09 with the packer fix** (md5 `35cdc402…`): WNS +0.167 ns, WHS +0.010 ns, 0 failing endpoints of 136,202, all timing constraints met; 36,165 LUTs, 35,170 FFs, 83 RAMB36, 15 RAMB18, 224 URAM. **On hardware the same day: the whole disc goes into HBM by DMA.** The 4.34 GiB Star Wars Battlefront II image stages in 6.3 s (0.74 GB/s, 2,225 chunks), and reading it back through the HBM probe finds the ISO9660 volume descriptor, the root directory, `SYSTEM.CNF` at LBA 2,265,115 and a valid ELF at LBA 2,265,116 — the far-end files, walked using only what HBM returned. 400 non-zero beats sampled from 24 windows spanning the image are byte-identical, the highest at 0x10a911920, past the 4 GiB mark. The fix in this image stops the packer carrying a beat across transfers, which had displaced every staged image by exactly 32 bytes ([hbm.md](../docs/hbm.md)). Supersedes md5 `3ac56952…`, whose entry read: Builds and closes timing: WNS +0.168 ns, 98,932 endpoints, 0 failing; 20,791 LUTs, 82 BRAM, 224 URAM. **PASS on hardware 2026-09-07:** `boot_test.s` reaches POST AA in 10 ms with all twelve stage writes, the pad-`0xFFFF` run fails at stage 09 as designed, five repeats pass (`docs/ps2-iop-bringup.md`). Predates the peek port and HBM. |
