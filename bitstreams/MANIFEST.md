...
| `c1100_ps2_iop.bit` | `8d9088167e4031608ed62caf2b5e832a` | xcu55n-fsvh2892-2LV-e | `boards/c1100_ps2_iop.py` — a LitePCIe gen3 x4 endpoint, the IOP subsystem on CSRs, HBM with three masters, both SIF DMA channels, block-mode DMA, and the IOP console | **Rebuilt 2026-09-10 with the SIF0 backpressure fix** (md5 `8d908816…`): WNS +0.028 ns — the tightest of this series, worth watching — 0 failing endpoints of 137,999, all timing constraints met. **On hardware the same day, on a stock BIOS: the console reads a whole file off the game disc.** all 166,708 bytes of `SLUS_212.40` are accounted for byte-identically across ~82 sectors of sequential reads through CDVDMAN: 166,704 by DMA and the final 4 in the read's fragment struct. Its tightest path is +0.028 ns, which is asynchronous reset recovery in the 250 MHz AXI domain and unrelated to anything in this change. Supersedes md5 `3ec5e985…`, whose SIF0 channel dropped words whenever a transfer was larger than its FIFO, so a 2 KB read arrived with a corrupt tail and a 16 KB read arrived not at all. |
