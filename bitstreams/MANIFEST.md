...
| `c1100_ps2_iop.bit` | `0440d304f9248ae11df259458d3c06f1` | xcu55n-fsvh2892-2LV-e | `boards/c1100_ps2_iop.py` — a LitePCIe gen3 x4 endpoint, the IOP subsystem on CSRs, HBM with three masters, both SIF DMA channels, and the DMA/INTC registers exposed to the host | **Rebuilt 2026-09-10 with the DMA master-interrupt fix** (md5 `0440d304…`): WNS +0.131 ns, 0 failing endpoints of 137,393, all timing constraints met. **On hardware the same day: the BIOS answers SIFCMD.** Sending SIF_CMD_INIT_CMD with an EE buffer address makes the IOP re-arm SIF1, program channel 9 and reply SIF_CMD_SET_SREG setting SREG[0] (RPCINIT) to 1, addressed to the buffer the host named — change the address and the reply follows it. Supersedes md5 `9232938e…`, which had SIF0 and the DMA read path but could raise no interrupt from any channel above 6, so the BIOS never saw a packet arrive. |
