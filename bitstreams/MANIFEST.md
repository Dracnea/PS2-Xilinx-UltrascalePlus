...
| `c1100_ps2_iop.bit` | `f33f8528d3f4fb0e8b5b0741ebacdeac` | xcu55n-fsvh2892-2LV-e | `boards/c1100_ps2_iop.py` — a LitePCIe gen3 x4 endpoint, the IOP subsystem on CSRs, HBM with three masters (the disc source, the IOP, and the host-to-card DMA writer) | **Rebuilt 2026-09-09 with the disc source's burst fix** (md5 `f33f8528…`): WNS +0.261 ns, WHS +0.010 ns, 0 failing endpoints of 136,153, all timing constraints met; 36,179 LUTs, 35,149 FFs, 83 RAMB36, 224 URAM. **On hardware the same day: the IOP reads the disc out of HBM with no host program running** — boot-test stage `0E` passes five times consecutively with `discserve.py` absent, and moving `iop_hbm_disc_base` so LBA 16 lands on the SYSTEM.CNF sector makes `0E` fail and puts `BOOT2 = cdrom0:\SLUS_212.40;1` in IOP RAM, which is what shows the read is real and that the address path works past 4 GiB ([hbm.md](../docs/hbm.md)). Supersedes md5 `35cdc402…`, whose entry read: rebuilt with the packer fix; the whole 4.34 GiB image stages in 6.3 s and 400 sampled non-zero beats are byte-identical, but the IOP had not yet read through it — the disc source asked HBM for a 64-beat burst, which its 4-bit ARLEN truncates to 16, and sat waiting for the seventeenth beat. |
